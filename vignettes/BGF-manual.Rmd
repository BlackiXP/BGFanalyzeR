...
A manual BGF can be created using BGF() and explicitly setting the MeasurementType argument.
...
Our `BGF` has been created and a lab experiment was successfully designed.
...
We will generate some random gas measurements for our example assuming that the fermentation has a low but constant gas production:
...
tail(myBGF$BioGasData)
```
Now that we entered product values for both fermentation, we can continue by calculating further standard fermentation parameters, like production or net gas.
